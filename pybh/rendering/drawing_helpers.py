...
class AxisDrawer(object):
    ...
    def __init__(self, camera, scale=8.0, line_width=5.0, viewport_fraction=1/4, background_color=None):
    ...
    def render(self, ctx):
        ...
        axis_view_matrix = self._camera.view_matrix_without_translation
        axis_view_matrix = np.dot(transforms.TransformMatrix.from_translation([0, 0, -10]).matrix, axis_view_matrix)
        ctx.disable(moderngl.DEPTH_TEST)
        self._axis.render(ctx, self._camera.projection_matrix, axis_view_matrix)
